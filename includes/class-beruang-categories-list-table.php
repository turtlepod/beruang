<?php
/**
 * WP_List_Table for admin categories list.
 *
 * @package Beruang
 */
...
/**
 * Categories list table for admin page.
 */
class Categories_List_Table extends \WP_List_Table {
	...
	public function __construct( $user_filter = 0 ) {
		...
		parent::__construct(
			array(
				...
				'plural'   => 'categories',
				'ajax'     => false,
				'screen'   => $screen ? $screen->id : 'beruang_page_beruang-categories',
			)
		);
	}
	...
	public function prepare_items() {
		...
		if ( ! empty( $_GET['orderby'] ) ) {
			$allowed = array( 'id', 'user_id', 'name', 'parent_id', 'sort_order' );
	...
	/**
	 * Render name column (with indent when filtered by single user for tree display).
	 *
	...
	protected function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'sort_order':
				return (string) ( $item['sort_order'] ?? 0 );
			default:
				return '';
		}
	}
	...
	protected function extra_tablenav( $which ) {
		...
		$user_ids      = DB::get_category_user_ids();
