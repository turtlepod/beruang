...
function admin_register_settings() {
	...
	register_setting( 'beruang_settings', 'beruang_currency', array(
		'type' => 'string',
		'default' => 'IDR',
		'sanitize_callback' => 'sanitize_text_field',
	) );
	register_setting( 'beruang_settings', 'beruang_decimal_sep', array(
		'type' => 'string',
		'default' => ',',
		'sanitize_callback' => 'sanitize_text_field',
	) );
	register_setting( 'beruang_settings', 'beruang_thousands_sep', array(
		'type' => 'string',
		'default' => '.',
		'sanitize_callback' => 'sanitize_text_field',
	) );
...
function admin_handle_export() {
	...
	header( 'Content-Disposition: attachment; filename="beruang-export-' . date( 'Y-m-d' ) . '.json"' );
...
function admin_handle_update_transaction() {
	...
	if ( ! $id ) {
		wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-transactions', 'beruang_error' => 'invalid' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	$existing = DB::get_transaction_by_id( $id );
	if ( ! $existing ) {
		wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-transactions', 'beruang_error' => 'notfound' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	$date = isset( $_POST['beruang_date'] ) ? sanitize_text_field( $_POST['beruang_date'] ) : $existing['date'];
	$time = isset( $_POST['beruang_time'] ) ? sanitize_text_field( $_POST['beruang_time'] ) : null;
	if ( $time === '' ) {
		$time = null;
	}
	$description = isset( $_POST['beruang_description'] ) ? sanitize_textarea_field( $_POST['beruang_description'] ) : '';
	...
	$type = isset( $_POST['beruang_type'] ) && $_POST['beruang_type'] === 'income' ? 'income' : 'expense';
	...
	wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-transactions', 'beruang_updated' => '1' ), admin_url( 'admin.php' ) ) );
	exit;
}
...
function admin_handle_update_category() {
	...
	if ( ! $id || ! $user_id ) {
		wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-categories', 'beruang_error' => 'invalid' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	$existing = DB::get_category_by_id( $id );
	if ( ! $existing || (int) $existing['user_id'] !== $user_id ) {
		wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-categories', 'user_id' => $user_id, 'beruang_error' => 'notfound' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	$name = isset( $_POST['beruang_cat_name'] ) ? sanitize_text_field( $_POST['beruang_cat_name'] ) : '';
	$parent_id = isset( $_POST['beruang_cat_parent'] ) ? absint( $_POST['beruang_cat_parent'] ) : 0;
	if ( $name === '' ) {
		wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-categories', 'user_id' => $user_id, 'edit' => $id, 'beruang_error' => 'name' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	DB::save_category( $user_id, array( 'name' => $name, 'parent_id' => $parent_id ), $id );
	wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-categories', 'user_id' => $user_id, 'beruang_updated' => '1' ), admin_url( 'admin.php' ) ) );
	exit;
}
...
function admin_handle_update_budget() {
	...
	if ( ! $id || ! $user_id ) {
		wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-budgets', 'beruang_error' => 'invalid' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	$existing = DB::get_budget_by_id( $id );
	if ( ! $existing || (int) $existing['user_id'] !== $user_id ) {
		wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-budgets', 'user_id' => $user_id, 'beruang_error' => 'notfound' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	$name = isset( $_POST['beruang_budget_name'] ) ? sanitize_text_field( $_POST['beruang_budget_name'] ) : '';
	$target_amount = isset( $_POST['beruang_budget_target'] ) ? floatval( $_POST['beruang_budget_target'] ) : 0;
	$type = isset( $_POST['beruang_budget_type'] ) && $_POST['beruang_budget_type'] === 'yearly' ? 'yearly' : 'monthly';
	...
	if ( $name === '' ) {
		wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-budgets', 'user_id' => $user_id, 'edit' => $id, 'beruang_error' => 'name' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	DB::save_budget( $user_id, array( 'name' => $name, 'target_amount' => $target_amount, 'type' => $type, 'category_ids' => $category_ids ), $id );
	wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-budgets', 'user_id' => $user_id, 'beruang_updated' => '1' ), admin_url( 'admin.php' ) ) );
	exit;
}
...
function admin_handle_add_budget_category() {
	...
	if ( $budget_id < 1 || $category_id < 1 ) {
		wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-budget-categories', 'beruang_error' => 'invalid' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	DB::insert_budget_category( $budget_id, $category_id );
	wp_safe_redirect( add_query_arg( array( 'page' => 'beruang-budget-categories', 'beruang_added' => '1' ), admin_url( 'admin.php' ) ) );
	exit;
}
...
function admin_handle_delete_budget_category() {
	...
	if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'beruang_delete_bc' ) ) {
	...
	$redirect = array( 'page' => 'beruang-budget-categories', 'beruang_deleted' => '1' );
...
function admin_page_transactions() {
	...
	<div class="wrap">
		...
		if ( isset( $_GET['beruang_error'] ) && $_GET['beruang_error'] === 'notfound' ) {
		...
		if ( $edit_row ) {
			...
			<div class="beruang-admin-edit-transaction" style="margin-bottom:1.5em;padding:1em;background:#f0f0f1;border-left:4px solid #2271b1;">
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'beruang-transactions', 'user_id' => $user_filter ?: null ), admin_url( 'admin.php' ) ) ); ?>" class="button"><?php esc_html_e( 'Cancel', 'beruang' ); ?></a></p>
				</form>
			</div>
			<?php
		}
		...
		<table class="wp-list-table widefat fixed striped">
			...
			if ( empty( $items ) ) {
				...
				foreach ( $items as $row ) {
					...
					$edit_url = add_query_arg( array( 'page' => 'beruang-transactions', 'edit' => $row['id'], 'user_id' => $user_filter ?: null ), admin_url( 'admin.php' ) );
...
function admin_page_categories() {
	...
	if ( isset( $_POST['beruang_add_category'] ) && check_admin_referer( 'beruang_category' ) && $user_filter ) {
		$name = sanitize_text_field( $_POST['beruang_cat_name'] ?? '' );
		$parent_id = absint( $_POST['beruang_cat_parent'] ?? 0 );
		if ( $name !== '' ) {
	...
	<div class="wrap">
		...
		<?php if ( isset( $_GET['beruang_error'] ) && $_GET['beruang_error'] === 'name' ) { echo '<div class="notice notice-error"><p>' . esc_html__( 'Name is required.', 'beruang' ) . '</p></div>'; } ?>
		<form method="get" class="beruang-admin-filter">
		...
		<div class="beruang-admin-edit-box" style="margin:1em 0;padding:1em;background:#f0f0f1;border-left:4px solid #2271b1;">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<p><button type="submit" class="button button-primary"><?php esc_html_e( 'Update', 'beruang' ); ?></button>
				<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'beruang-categories', 'user_id' => $user_filter ), admin_url( 'admin.php' ) ) ); ?>" class="button"><?php esc_html_e( 'Cancel', 'beruang' ); ?></a></p>
			</form>
		</div>
		<?php } ?>
		...
		<table class="wp-list-table widefat fixed striped">
			...
			if ( empty( $categories ) ) {
				...
				foreach ( $categories as $row ) {
					$edit_url = add_query_arg( array( 'page' => 'beruang-categories', 'user_id' => $user_filter, 'edit' => $row['id'] ), admin_url( 'admin.php' ) );
					$del_url = wp_nonce_url( add_query_arg( array( 'page' => 'beruang-categories', 'user_id' => $user_filter, 'delete' => $row['id'] ) ), 'beruang_delete_cat_' . $row['id'] );
...
function admin_page_budgets() {
	...
	<div class="wrap">
		...
		<?php if ( isset( $_GET['beruang_error'] ) && $_GET['beruang_error'] === 'notfound' ) { echo '<div class="notice notice-error"><p>' . esc_html__( 'Budget not found.', 'beruang' ) . '</p></div>'; } ?>
		<?php if ( isset( $_GET['beruang_error'] ) && $_GET['beruang_error'] === 'name' ) { echo '<div class="notice notice-error"><p>' . esc_html__( 'Name is required.', 'beruang' ) . '</p></div>'; } ?>
		<form method="get" class="beruang-admin-filter">
		...
		<div class="beruang-admin-edit-box" style="margin:1em 0;padding:1em;background:#f0f0f1;border-left:4px solid #2271b1;">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<p><button type="submit" class="button button-primary"><?php esc_html_e( 'Update', 'beruang' ); ?></button>
				<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'beruang-budgets', 'user_id' => $user_filter ), admin_url( 'admin.php' ) ) ); ?>" class="button"><?php esc_html_e( 'Cancel', 'beruang' ); ?></a></p>
			</form>
		</div>
		<?php } ?>
		<table class="wp-list-table widefat fixed striped">
			...
			if ( empty( $budgets ) ) {
				...
				foreach ( $budgets as $row ) {
					...
					$edit_url = add_query_arg( array( 'page' => 'beruang-budgets', 'user_id' => $user_filter, 'edit' => $row['id'] ), admin_url( 'admin.php' ) );
	...
	<div class="wrap">
		...
		<table class="wp-list-table widefat fixed striped">
			...
			if ( empty( $items ) ) {
				...
				foreach ( $items as $row ) {
					$delete_args = array( 'action' => 'beruang_delete_budget_category', 'budget_id' => $row['budget_id'], 'category_id' => $row['category_id'] );
					if ( $user_filter > 0 ) {
